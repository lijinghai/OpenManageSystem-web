...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="专业代码" width="120px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.major_code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="专业名称" width="120px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.major_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="层次" width="120px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.level}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" style="width: 400px; margin-left:150px;">
        <el-form-item :label="$t('table.major_code')" prop="major_code">
          <el-input v-model="temp.major_code" />
        </el-form-item>

        <el-form-item :label="$t('table.major_name')" prop="major_name">
        ...
        <el-form-item :label="$t('table.major_note')" prop="major_note">
          <el-input v-model="temp.major_note" />
        </el-form-item>
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle, deleteUser } from '@/api/major'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        major_code: [{ required: true, message: '专业代码必须输入', trigger: 'change' }],
        major_name: [{ required: true, message: '专业层次名称必须输入', trigger: 'change' }],
        level: [{ required: true, message: '层次必须输入', trigger: 'change' }],
        major_note: [{ required: true, message: '备注称必须输入', trigger: 'change' }]
      },
