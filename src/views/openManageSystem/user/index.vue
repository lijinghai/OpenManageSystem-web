...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="邮箱" width="120px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.email }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" style="width: 400px; margin-left:150px;">
        ...
        <el-form-item :label="$t('table.code1')" prop="code">
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle, deleteUser } from '@/api/openuser'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        name: [{ required: true, message: '姓名必须输入', trigger: 'change' }],
        code: [{ required: true, message: '编号必须输入', trigger: 'change' }],
        phone: [{ required: true, message: '手机必须输入', trigger: 'change' }],
        email: [{ required: true, message: '邮箱必须输入', trigger: 'change' }],
        role: [{ required: true, message: '角色必须输入', trigger: 'change' }]
      },
