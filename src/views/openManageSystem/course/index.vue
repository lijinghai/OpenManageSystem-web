...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="课程类别代码" width="120px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.course_code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="课程类别名称" width="120px" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" style="width: 400px; margin-left:150px;">
        <el-form-item :label="$t('table.course_code')" prop="course_code">
          <el-input v-model="temp.course_code" />
        </el-form-item>
        <el-form-item :label="$t('table.course_name')" prop="course_name">
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle, deleteUser } from '@/api/course'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        course_code: [{ required: true, message: '课程类别代码必须输入', trigger: 'change' }],
        course_name: [{ required: true, message: '课程类别名称必须输入', trigger: 'change' }]
      },
