...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="资源分类代码" width="120px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.resource_code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="资源分类名称" width="120px" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" style="width: 400px; margin-left:150px;">
        <el-form-item :label="$t('table.resource_code')" prop="resource_code">
          <el-input v-model="temp.resource_code" />
        </el-form-item>
        <el-form-item :label="$t('table.resource_name')" prop="resource_name">
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle, deleteUser } from '@/api/resource'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        resource_code: [{ required: true, message: '资源分类代码必须输入', trigger: 'change' }],
        resource_name: [{ required: true, message: '资源分类名称必须输入', trigger: 'change' }]
      },
