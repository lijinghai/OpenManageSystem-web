...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="学制" width="120px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle, deleteUser } from '@/api/educate'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        code: [{ required: true, message: '学制必须输入', trigger: 'change' }]
      },
